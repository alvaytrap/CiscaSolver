...
const schema = z.object({
  cacheLines: z
    ...
    .int()
    .refine((value) => value !== undefined, { message: "Este campo es requerido" }),
  wordsPerLine: z
    .number()
    .min(1, "Debe ser un número positivo")
    .int()
    .refine((value) => value !== undefined, { message: "Este campo es requerido" }),
  addressesToCheck: z.string().refine((value) => value !== undefined, { message: "Este campo es requerido" }),
});

const CacheSimPage = () => {
  const [cacheLines, setCacheLines] = useState(4);
  const [wordsPerLine, setWordsPerLine] = useState(16);
  const [addressesToCheck, setAddressesToCheck] = useState(
  ...
  return (
    <Container>
      ...
      >
      <Typography variant="h4" sx={{m:2}}>Simulador de Caché</Typography>
            ...
            render={({ field, fieldState }) => (
              <>
                <TextField
                  ...
                  variant="outlined"
                  value={cacheLines}
                  onChange={(e) => setCacheLines(+e.target.value)}
                  fullWidth
                />
                {fieldState?.error && (
            ...
            render={({ field, fieldState }) => (
              <>
                <TextField
                  ...
                  onChange={(e) => setAddressesToCheck(e.target.value)}
                  fullWidth
                />
                {fieldState?.error && (
                  <span className="error">{fieldState?.error?.message}</span>
